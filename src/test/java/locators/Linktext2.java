package locators;

import java.util.concurrent.TimeUnit;

import org.openqa.selenium.By;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.chrome.ChromeDriver;

public class Linktext2 {

	public static void main(String[] args) throws InterruptedException 
	{
		System.setProperty("webdriver.chrome.driver","D:\\shami\\chromedriver.exe");
		WebDriver sravan=new ChromeDriver();
		sravan.manage().timeouts().implicitlyWait(30, TimeUnit.SECONDS);
		sravan.get("https://rahulshettyacademy.com/locatorspractice/");
		Thread.sleep(3000);
		sravan.findElement(By.linkText("Forgot your password?")).click(); 
		Thread.sleep(3000);
		sravan.findElement(By.xpath("//input[@type='text'][1]")).sendKeys("sravan");
		sravan.findElement(By.xpath("//input[@type='text'][2]")).sendKeys("lakhan");
		sravan.findElement(By.xpath("//input[@type='text'][3]")).sendKeys("9642883037");
		Thread.sleep(3000);
		sravan.findElement(By.xpath("//button[@class='go-to-login-btn']")).click();
		Thread.sleep(3000);
		sravan.quit();
	}  }
